...
static void quantize_row_q8_KS(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
...
void ggml_compute_forward_mul_mat(
    ...
    if (dst->flags & GGML_TENSOR_FLAG_IKQ) {
        ...
        assert(ne13 == 1 && ne12 == 1); // assume the activation is also two dimentions
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    if (tensor->op == GGML_OP_MUL_MAT && res_tensor) {
        ggml_barrier(params->threadpool);
        const int ith = params->ith;
        const int nth = params->nth;
