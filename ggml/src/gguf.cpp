...
struct gguf_context * gguf_init_from_file_impl(FILE * file, struct gguf_init_params params) {
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor type
        {
            ...
            if (info.t.type == GGML_TYPE_EXL3) {
                info.t.nb[1] = info.t.nb[0] * (info.t.ne[0] + info.t.ne[1] +
                    (info.t.ne[0]/blck_size)*(info.t.ne[1]/blck_size)*info.t.ne[2]);
                for (int j = 2; j < GGML_MAX_DIMS; ++j) {
                    info.t.nb[j] = info.t.nb[j - 1];
                }
            } else {
